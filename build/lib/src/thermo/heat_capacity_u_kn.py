...
TE_COL_NUM = 4  #The column number of the total energy in ener_box#.output
...
T_from_file = numpy.array([300.0+10.0*i for i in range(NumTemps)])
...
E_kn_samp = numpy.zeros([K,NumIterations], numpy.float64) # E_kn is total ener. of segment n evaluated at temp k
...
for n in range(nBoots_work):
    ...
    for k in range(K):
      length = 1
      for l in range(K):
        u_kn[k,length-1:length-1+Nall_k[l]] = beta_k[k] * E_kn_samp[l,0:Nall_k[l]]
        length = length + Nall_k[l]
